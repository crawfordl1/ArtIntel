...
TOPIC: Determine the Cross Track Error for the Robot moving through the Grid
--Robot follows a series of Linear nodes, CTE must accommodate the transition from one node to the next
---Collisions are possible and the Robot Path adjusted for
'''
# -----------
# User Instructions
#
...
class robot:
    ...
    '''### MOVE() FUNCTION ###
    -Purpose: Measurement Update of Robot moving between grid-cells
    -Input: 
        -Grid - Robot environment for Robot to navigate through
        -Steering - front wheel steering angle, limited by max_steering_angle
        -Distance - total distance driven, most be non-negative
        -Tolerance - small static integer compare turning error to
        -Steering Angle - angle to accommodate for as Robot moves between grids
    -Output: 
        -Res - X, Y and Orientation of Robot's Location after the Move
    '''    
    # --------
    # move: 
    #    steering = front wheel steering angle, limited by max_steering_angle
    ...
    '''### SENSE() FUNCTION ###
    -Purpose: Random indication of where the Robot "thinks" it is on the grid
        -*Note: Functions allow for the calculation of any arbitrary, non-specified Input
    -Input: 
        -Current location meta-data
    -Output: 
        -Random X & Y Location and associated Random noise
    '''
    def sense(self):
...
class particles:
    ...
    # --------
    #
    #Sense Measurements to determine Bearing and then Resample
    # 
...
--As Robot gets closer to Trajectory, it "counter-steers" in order to not Overshoot
-Output: 
    -Did the Robot find the Goal?
    -Did it hit an object along the way?
    -How many steps?
'''
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        #Start with the present robot estimate & Compute CTE
        estimate = filter.get_position()

        '''
        EXERCISE CODE WIRTTEN BELOW
        '''
        dx = spath[index + 1][0] - spath[index][0]  #Delta X = End of Line Segment (P2) minus Beginning (P1)
        dy = spath[index + 1][1] - spath[index][1]  #Delta Y = End of Line Segment (P2) minus Beginning (P1)
        
        drx = estimate[0] - spath[index][0]     #Rx = Robot Location from beginning of Line Segment (P1)
        dry = estimate[1] - spath[index][1]     #Ry = Robot Location from beginning of Line Segment (P1)
        
        #How far has the Robot progressed along the current Line Segment?
        u = (drx * dx + dry * dy) / (dx * dx + dy * dy)     #Determined by Dot Product
        
        #Cross Track Error for Robot distance away from Trajectory Line Segment
        cte = (dry * dx - drx * dy) / (dx * dx + dy * dy)
        
        #If larger than 1, Robot has LEFT the current Line Segment and is now traversing the next one
        if u > 1.0:
            index = index + 1
...
'''### MAIN() FUNCTION ###
    -Purpose: Utilize PD Controller with A* Searching in order to find an Optimized Smooth Path to Goal
    -Output: 
...
weight_data       = 0.1
weight_smooth     = 0.2
p_gain            = 2.0     #Proportional (Steer in Proportion to Minimized Error)
d_gain            = 6.0     #Differential (Avoid OVershooting Trajectory)
...
'''### TWIDDLE() FUNCTION ###
    ...
    -Input: 
        -Initial parameters before Twiddle Optimization
    -Output: 
