...
'''### SLAM() FUNCTION ###
    -Purpose: Determine Best Robot & Landmark Locations based on the Constraints in the Robot World
    -Input: 
        -data - Sequences of Motions and Measurements relative to input parameters
        -N - Length of the Path
        -num_landmarks - Number of Total Landmarks in the World
        -motion_noise
        measurement_noise  
    -Output: 
        -New Omega & Xi values with accompanying Results detailing Best Estimates for all Robot & Landmark locations
'''
...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        #Iterate through Measurement and Motion data values
        measurement = data[k][0]
        motion = data[k][1]
        ...
        for i in range(len(measurement)):
            ...
            # 'm' is the Index of the landmark coordinate in the matrix / vector
            g = 2 * (1 + measurement[i][0])
            
            # update the information matrix /vector based on the measurement
            for b in range(2):
                Omega.value[b][b] +=  1.0 / measurement_noise
                Omega.value[g+b][g+b] +=  1.0 / measurement_noise
                Omega.value[b][g+b] += -1.0 / measurement_noise
                Omega.value[g+b][b] += -1.0 / measurement_noise
                
                Xi.value[b][0]      += -measurement[i][1+b] / measurement_noise
                Xi.value[g+b][0]      +=  measurement[i][1+b] / measurement_noise
            
        
        '''
        -Add Space for next Robot "Pose"
        --Create a list to Expand the information matrix / vector by one new position
        ---[0, 1] maintains the original Robot pose
        '''        
        list = [0, 1] + range(4, dim+2)     #Add 2 Rows and 2 Columns for the next Robot Pose
        ...
        ###Specifically UPDATE the 2 new Rows 7 Columns
        for f in range(4):
            Omega.value[f][f] +=  1.0 / motion_noise
        for y in range(2):
            Omega.value[y][y+2] += -1.0 / motion_noise
            Omega.value[y+2][y] += -1.0 / motion_noise
            
            Xi.value[y][0] += -motion[y] / motion_noise
            Xi.value[y+2][0] +=  motion[y] / motion_noise

        #Factor out the previous Robot Pose
        newlist = range(2, len(Omega.value))
        a = Omega.take([0,1], newlist)
        j = Omega.take([0,1])
        c = Xi.take([0,1], [0])
        
        #Apply Factorization for A, B, C Matrices
        ##Take() remove previous Pose
        Omega = Omega.take(newlist) - a.transpose() * j.inverse() * a
        Xi = Xi.take(newlist, [0]) - a.transpose() * j.inverse() * c
