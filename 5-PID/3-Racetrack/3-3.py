...
class robot:
    ...
    '''### CTE() FUNCTION ###
    -Purpose: As Robot follows RaceTrack (RT) via Run(), calculate Cross Error Check to determine its ACCURACY
    -Input: 
        -Radius - radius of the RaceTrack
    -Output: 
        -Current Robot Cross Track Error(CTE) according to current Location
    '''
    def cte(self, radius):
        '''
        If on LEFT Side of RT, determine the Distance from the X Coordinate to the Radius
        -->Then, subtract that distance difference from the Radius itself
        -->If 100% ACCURATE, and the Robot is on the RT circle, CTE = 0
        '''
        if self.x < radius:
            cte = sqrt((self.x - radius) ** 2 + (self.y - radius) ** 2) - radius
    #If on RIGHT side of RT (distance from LEFT to RIGHT defined as: 3.0 * radius)
    #-->Then subtract that amount from the CTE calculation
        elif self.x > 3.0 * radius:
            cte = sqrt((self.x - 3.0 * radius) ** 2 + (self.y - radius) ** 2) - radius
    #If on TOP of RT, subtract the height of the RT from the Y coordinate to find the DIFFERENCE
        elif self.y > radius:
            cte = self.y - 2.0 * radius
    #If on Bottom of RT, Robot is on the Y-Axis
    #-->Include ( - ) sign b/c Robot is moving in Counter Clockwise Motion
        else:
            cte = -self.y         
        return cte
...
radius = 25.0               #Radius of the Race Track
params = [10.0, 15.0, 0]    #No Twiddle applied
