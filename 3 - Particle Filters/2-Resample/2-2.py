...
TOPIC: Loop Resample function multiple times to achieve Accurate Orientation measurements 
-i.e. Measurements: (x, y, z); where is orientation after x & y axis
-SUPPLEMENTARY: Also print the Eval() function to view Measurement Error of Mean
'''
...
T = 10       #Amount to Loop entire Particle Resampling for Accurate Orientation Measurements
...
for t in range(T):    
    ...
    index = int(random.random() * N)                    #Set Index to random Particle
    mw = max(w)
    for i in range(N):
        beta = beta + random.random() * 2.0 * mw    #Add Random Weight to Beta variable
        '''
    ...
    p1 = p3
    '''
    -Print Eval() T times to see accuracy progression
    '''
    print eval(myRobot, p1)
print p1